...
import servicioclientes.org.datacontract.schemas._2004._07.denariuscorebanking_web_externalservices.DatosCliente;
...
public class ServicioClientesDenarius {
	...
	public List<DatosClienteMS> consultarClienteCondicion(Integer tipoConsulta, String apellidoMaterno, String apellidoPaterno, String nombres, String identificacion, Integer idCliente) throws Exception {

		MEConsultaClientesCondiciones request = new MEConsultaClientesCondiciones();
		MSConsultaClientesCondiciones response;

		servicioclientes.org.datacontract.schemas._2004._07.denariuscorebanking_web_externalservices.ObjectFactory ofCliente = new servicioclientes.org.datacontract.schemas._2004._07.denariuscorebanking_web_externalservices.ObjectFactory();

		request.setCabecera(cabeceraServicioCliente());
		request.setTipoConsulta(tipoConsulta);
		request.setIdentificacion(ofCliente.createMEConsultaClientesCondicionesIdentificacion(identificacion));
		request.setApellidoMaterno(ofCliente.createMEConsultaClientesCondicionesApellidoMaterno(apellidoMaterno));
		request.setApellidoPaterno(ofCliente.createMEConsultaClientesCondicionesApellidoPaterno(apellidoPaterno));
		request.setNombres(ofCliente.createMEConsultaClientesCondicionesNombres(nombres));
		request.setIdCliente(idCliente);
		
		// llamado a ws denarius
		response = interfazClientes.consultarClientesCondiciones(request);

		if (Boolean.TRUE.equals(response.isOk())) {
			List<DatosClienteMS> lista = new ArrayList<>();
			for (DatosCliente cliente : response.getClientes().getValue().getDatosCliente()) {
				lista.add(new DatosClienteMS(cliente.getCliente().getValue().getIdCliente(), 
						cliente.getCliente().getValue().getTipoIdentificacion().toString(),
						cliente.getCliente().getValue().getIdentificacion().getValue(), null, null,
						cliente.getCliente().getValue().getNombres().getValue(),
						cliente.getCliente().getValue().getPrimerApellido().getValue(),
						cliente.getCliente().getValue().getSegundoApellido().getValue(), null,
						cliente.getCliente().getValue().getNombresCompletos().getValue(), null, null,
						cliente.getCliente().getValue().getEstadoCivil().getValue(),
						cliente.getTelefonoCelular().getValue(),
						cliente.getCliente().getValue().getEmail().getValue(),
						cliente.getTelefonoLocal().getValue(),
						DateUtils.fechaToString(cliente.getCliente().getValue().getFechaNacimiento()),
						cliente.getCliente().getValue().getNivelEstudios().getValue(),
						cliente.getDatosDireccion().getValue().getTipoVivienda().getValue(),
						cliente.getCliente().getValue().getGenero().toString(),
						cliente.getCliente().getValue().getCodigoActividad().getValue(),
						cliente.getDatosDireccion().getValue().getCiudad().getValue(),
						cliente.getDatosDireccion().getValue().getDireccion().getValue(), null, null, null, null, null, null, null, null, null, null, null, null, null, null));

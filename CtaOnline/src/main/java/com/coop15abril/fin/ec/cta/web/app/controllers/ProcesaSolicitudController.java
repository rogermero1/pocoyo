...
public class ProcesaSolicitudController {
	...
	public ResponseEntity<?> procesaCliente(@Valid @RequestBody Cliente PersonaDatos, BindingResult result) throws Exception {
       ...
       if ( result.hasErrors() ) {
	    ...
	    if ( !extrasService.EnviarEmail(PersonaDatos.getApellidoUno()+" "+PersonaDatos.getApellidosDos() + " " +PersonaDatos.getNombres(), PersonaDatos.getEmail(),cuentaAhorro.getCuenta(), correoEmisor) ) {
	    	 logger.error("Error de envio de correo para el email:"+PersonaDatos.getEmail()+",  con numero de cuenta "+cuentaAhorro.getCuenta());
	    }
